...
export class DepartmentComponent {
  ...
  displayedColumns: string[] = ['code', 'name', 'unitCode', 'description', 'action'];
  ...
  getDept(data?: PageEvent) {
    return this.service.getDepartment().pipe(
      delay(300),
      map(department => {
    ...
    confirmDelete.afterClosed().subscribe(result => {
      if (result == 'true') {
        this.service.deleteDepartment(data).subscribe(
          (res: any) => {
            if (res.success) {
              this.notification.openSnackBar('Department Deleted Successfully', 2);
  ...
  edit(data: any) {
    this.dialog.open(ViewDeptComponent, { data: data });
  }
  ...
  addNew() {
    ...
    dialogConfig.disableClose = true;
    dialogConfig.autoFocus = true;
    let dept = this.dialog.open(ViewDeptComponent, dialogConfig);
