 ...
 sortData(data: any) {
  ...
  deleteItem(data: any) {
    ...
    confirmDelete.afterClosed().subscribe(result => {
      if (result == 'true') {
        this.service.deleteType2(data).subscribe(
          (res: any) => {
            if (res.success) {
              this.notification.openSnackBar('Business Partner Type 2 Deleted Successfully', 2);
              this.type2$=this.getType2();
            } else return;
          },
          (error: any) => {
            this.notification.openSnackBar('An error occurred while deleting the Business Partner Type 2', 2);
          }
        );
      }
    })
