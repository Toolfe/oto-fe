...
export class AddType1Component implements OnInit {
  ...
  updateType1() {
    ...
    this.contact.updateType1(this.data).subscribe(res => {
      ...
      this.notification.openSnackBar('Business Partner Type1 Updated Successfully', 1);
    })
  ...
  public addItem(): any {
    if (this.contactType1.valid == true) {
      ...
      this.contact.postType1(dataRow).subscribe(res => {
        this.dialogRef.close(res);
        this.notification.openSnackBar('Business Partner Type1 Added Successfully', 1)
      })
