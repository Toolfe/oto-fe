...
import { AddBusinessFunctionalityComponent } from './add-business-functionality/add-business-functionality.component';

@Component({
  selector: 'app-business-functionality',
  templateUrl: './business-functionality.component.html',
  styleUrls: ['./business-functionality.component.scss']
})
export class BusinessFunctionalityComponent  {
  ...
  dataSource!: MatTableDataSource<BusinessFunctionality>;
    ...
    confirmDelete.afterClosed().subscribe(result => {
      if (result == 'true') {
        this.service.deleteFunctionality(data).subscribe(
          (res: any) => {
            if (res.success) {
              this.notification.openSnackBar('Business Functionality Deleted Successfully', 2);
              this.dataSource.data = this.dataSource.data.filter(x => x.id !== data.id);
            } else return;
          },
          (error: any) => {
            this.notification.openSnackBar('An error occurred while deleting the Business Functionality', 2);
          }
        );
      }
    })
  ...
  edit(data: any) {
    this.dialog.open(AddBusinessFunctionalityComponent, { data: data });
  }
  ...
  addNew() {
    ...
    dialogConfig.disableClose = true;
    dialogConfig.autoFocus = true;
    let functionality=this.dialog.open(AddBusinessFunctionalityComponent, dialogConfig);
